...
```json
{
    ...
    "plain_talk_short": "一句话大白话：短期怎么做？（限30字以内，如'跌破10块就跑，站稳11块再买'）",
    "plain_talk_long": "一句话大白话：长期怎么拿？（限30字以内，如'业绩没雷，可以拿长线，目标看新高'）",
...
8. **拒绝偷懒**：遇到无新闻或无财报数据时，必须基于PE/PB、市值、换手率等现有数据进行估值和活跃度分析，**严禁使用'暂无数据'、'无重大消息'等敷衍话术**作为独立段落。
9. **大白话总结**：**必须填写 plain_talk_short 和 plain_talk_long**。这是显示在屏幕底部的“一句话攻略”，必须极简、通俗、直接。禁止使用任何术语，严格控制在30字以内。"""
