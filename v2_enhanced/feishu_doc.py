...
class FeishuDocManager:
    ...
    def is_configured(self) -> bool:
        """检查配置是否完整"""
        return bool(HAS_LARK and self.app_id and self.app_secret and self.folder_token)

    def create_daily_doc(self, title: str, content_md: str) -> Optional[str]:
        """
        创建日报文档
        """
        if not HAS_LARK or not self.client or not self.is_configured():
            logger.warning("飞书 SDK 未安装或配置缺失，跳过创建")
            return None
