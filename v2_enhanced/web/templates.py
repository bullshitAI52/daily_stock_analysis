...
<script>
    ...
    function renderTaskCard(taskId, taskData) {
        ...
        return `<div class="task-card ${status}" id="task_${taskId}" onclick="toggleDetail('${taskId}')">
            ...
            <div class="task-actions">
                ${status === 'completed' ? `
                <button class="task-btn" onclick="event.stopPropagation();window.location.href='/report/download?code=${code}&type=summary'" title="下载极简日报(Markdown)">📝</button>
                <button class="task-btn" onclick="event.stopPropagation();window.location.href='/report/download?code=${code}&type=detail'" title="下载深度报告(Markdown)">📊</button>
                <button class="task-btn" onclick="event.stopPropagation();window.location.href='/report/download?code=${code}&type=plain_talk'" title="下载大白话版(Markdown)">💬</button>
                <button class="task-btn" onclick="event.stopPropagation();window.location.href='/report/download?code=${code}&type=zip'" title="打包下载全部报告(ZIP)">📦</button>
                ` : ''}
    ...
    window.exportToPDF = function(taskId, code, name) {
        ...
        if (!detailEl) {
            alert('无法找到分析详情，请先展开任务查看详情后再导出PDF');
            return;
        }
        
        // 检查内容是否为空
        if (!detailEl.textContent || detailEl.textContent.trim().length < 50) {
            alert('分析详情内容为空或过短，无法生成PDF');
            return;
        }
        ...
        // 标题头
        const title = `<h3 style="margin-bottom: 10px;">${code} ${name} - 投资分析报告</h3>`;
  ...
  <div class="container">
    ...
    <div class="analysis-section" style="margin-top: 0; padding-top: 0; border-top: none;">
      ...
      <details class="usage-tips" style="margin-bottom: 1rem; padding: 0.75rem; background: #f0f9ff; border: 1px solid #bae6fd; border-radius: 0.5rem; cursor: pointer;">
        <summary style="font-weight: 500; color: #0369a1; user-select: none; outline: none;">
          💡 使用提示
        </summary>
        <div style="margin-top: 0.5rem; padding-top: 0.5rem; border-top: 1px solid #bae6fd; font-size: 0.875rem; color: #0c4a6e; line-height: 1.6;">
          <p style="margin: 0 0 0.5rem 0;">
            <strong>✨ 批量分析：</strong>可以同时输入多个股票代码进行分析，无需等待上一个完成！
          </p>
          <p style="margin: 0 0 0.5rem 0;">
            <strong>📊 报告类型：</strong>
          </p>
          <ul style="margin: 0; padding-left: 1.5rem;">
            <li><strong>完整报告</strong>：包含多维度情报搜索，分析更全面（约5分钟）</li>
            <li><strong>精简报告</strong>：仅基于技术面和基本面，速度更快（约1分钟）</li>
          </ul>
          <p style="margin: 0.5rem 0 0 0;">
            <strong>📦 下载报告：</strong>分析完成后，点击任务卡片展开详情，可下载 MD 格式报告或打包下载全部
          </p>
        </div>
      </details>
    ...
    <div class="footer">
      <div id="footer_default" style="display: none;">
      </div>
